...
vector< bitset<NUM_BITS>> RegistradorReset(const vector< bitset<NUM_BITS>> &input, const vector< bitset<NUM_BITS_CONTROL>> &ck,
                                           const vector< bitset<NUM_BITS_CONTROL>> &reset)
{
    ...
    for(unsigned int i = 1; i < NUM_CASES_TEST; i++){
        ...
        if((ck[i - 1].to_string() == "0" && ck[i].to_string() == "1") && !reset[i].test(NUM_BITS_CONTROL - 1))
            output[i] = input[i];
...
void FileRegistradorReset(const string filename, const vector< bitset<NUM_BITS>> &input, const vector< bitset<NUM_BITS_CONTROL>> &ck,
                          const vector< bitset<NUM_BITS_CONTROL>> &reset)
{
    ofstream fileGold;
    ...
    output = RegistradorReset(input, ck, reset);

    for(unsigned int i = 0; i < NUM_CASES_TEST; i++){
        if(!i)
            fileGold << ck[i] << "_" << reset[i] << "_" << input[i] << "_" << "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx" << endl;
        else
            fileGold << ck[i] << "_" << reset[i] << "_" << input[i] << "_" << output[i] << endl;
    }
...
int main(void)
{
    ...
    FileRegistradorReset("registradorReset.tv", inputs, cks, resets);
